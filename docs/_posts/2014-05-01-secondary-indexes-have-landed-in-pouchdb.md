...
```js
// design doc that tells PouchDB/CouchDB
...
pouch.query(myMapFunction, {key: 'Pika pi!', include_docs: true});

// find the first 5 pokemon whose name starts with 'P'
pouch.query(myMapFunction, {startkey: 'P', limit: 5, include_docs: true});
```
...
All you need to do is `put()` this document into your database, and then it will be available for querying using the name `my_index`.

Whatever name you give, be sure to use the same one in both the `_id` (after `'_design/'`) and in the `views`; otherwise you will need to use the format `'my_design_doc_name/my_view_name'` when you query.
...
From now on, you can call `pouch.query('my_index')` and it will summon this design document for querying.  You can always delete or change the design doc later; the index will be updated automatically the next time you `query()`.

If you do this a lot, though, then old indexes can build up on disk.  PouchDB and CouchDB will clean up these orphaned indexes for you if you call `pouch.viewCleanup()`. 

{% include alert_start.html variant="info"%}

<strong>Why design docs?</strong> Design docs are synced, stored, updated, and deleted exactly like normal documents.  They also show up in <code>allDocs()</code> queries, but you can filter them out by using <code>{startkey: 'design0'}</code>.  A design doc can contain multiple views, but there's really no advantage to this, and it can even cause performance problems in CouchDB, because all view indexes are stored in a single file. So by default, PouchDB only creates one view per design doc, and we use the same name for both.
...
pouch.put(designDoc).catch(function (err) {
  ...
  pouch.query('my_index', {key: 'foobar'}).then(function (result) {
    // got the 'foobar' doc
  });
});
```
...
Of course, some of our loyal Pouchinistas may have been perfectly happy with the old way of doing things. The fact that the old `query()` function slurped up the entire database into memory might have served them just fine, thank you.
...
But in general, we strongly recommend upgrading your apps to the new map/reduce API.
...
For instace, say your documents are emails that you want to sort by recency: boom, just set the `_id` to `new Date().toJSON()`.  Or say they're web pages that you want to look up by URL: use the URL itself as the `_id`! Neither CouchDB nor PouchDB have a limit on how long your `_id`s can be, so you can get as fancy as you want with this.
...
```js
allDocs({startkey: 'album_dylan_1964_', endkey: 'album_dylan_1965__', descending: true});
```

In this example, you're getting all those "indexes" for free, each time a document is added to the database.  It doesn't take up any additional space on disk compared to UUIDs, and you don't have to wait for a view to get built up, nor do you have to understand the map/reduce API at all.

Of course, this system starts to get shaky when you need to search by a variety of criteria: e.g. all albums sorted by year, artists sorted by age, etc. And you can only sort strings &ndash; not numbers, booleans, arrays, or arbitrary JSON objects, like the map/reduce API supports.  But in a lot of cases, you can get by without using the `query()` API at all.
